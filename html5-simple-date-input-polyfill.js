(function () {
  ...
  function CalendarExtender (theInput, i18n) {
    if (!(this instanceof CalendarExtender)) {
      return new CalendarExtender(theInput);
    }
    var self = this;
    ...
    this.init = function () {
      this.setMonthNames(i18n && i18n.monthNames);
      this.setWeekDaysAbbreviated(i18n && i18n.weekDaysAbbreviated);
    ...
    this.creathYearAndMonthSelects = function () {
      ...
      var monthSelect = this.createRangeSelect(0, 11, this.selectedDate.getMonth(), this.monthNames);
    ...
    this.setWeekDaysHeader = function () {
      this.weekDaysHeader = this.weekDaysAbbreviated.reduce(function (_, wd) {
    ...
    this.setWeekDaysAbbreviated = function (weekDaysAbbrev) {
      this.weekDaysAbbreviated = weekDaysAbbrev || ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
      this.setWeekDaysHeader();
    };

    this.setMonthNames = function (monthNames) {
      this.monthNames = monthNames || ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
    };
    ...
    this.createMonthTable = function () {
      ...
      theTable.appendChild(this.weekDaysHeader.cloneNode(true));
      ...
      for (var cellNum = 0; cellNum < maxDays + startDay; cellNum++) {
        ...
        if (cellNum + 1 > startDay) {
          ...
          aCell.addEventListener('click', function () {
            ...
            self.selectedDate.setDate(parseInt(this.textContent));
            self.selectDate();
            self.theInput.focus();
          });
        }
  ...
  // return false if the browser doesn't support input[type=date]
  ...
  // will add `CalendarExtender` to all inputs in the page
  function addCalendarExtenderToDateInputs (i18n) {
    // get and loop through all the input[type=date]s in the page that dont have "haveCal" class yet
    var dateInputs = document.querySelectorAll('input[type=date]:not(.haveCal)');
    [].forEach.call(dateInputs, function (dateInput) {
      // invoke CalendarExtender class on the input
      CalendarExtender(dateInput, i18n);
